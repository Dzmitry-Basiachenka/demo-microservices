...
public class ResourceControllerTest {
    ...
    void shouldGetResource() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get(URL_PATH + "/{id}", id))
            ...
            .andExpect(jsonPath("$.storageId").value(resourceEntity.getStorageId().intValue()))
    ...
    void shouldDownloadResource() throws Exception {
        ...
        resourceEntity.setStorageId(1000L);
    ...
    @Test
    void shouldCompleteResourceUpload() throws Exception {
        Long id = 1L;
        Long storageId = 1000L;

        when(resourceService.completeResourceUpload(id)).thenReturn(new ResourceCompletedResponse(id, storageId));

        mockMvc.perform(MockMvcRequestBuilders.put(URL_PATH + "/{id}/complete", id)
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.id").value(id.intValue()))
            .andExpect(jsonPath("$.storageId").value(storageId.intValue()));

        verify(resourceService).completeResourceUpload(id);
        verifyNoMoreInteractions(resourceService);
    }
