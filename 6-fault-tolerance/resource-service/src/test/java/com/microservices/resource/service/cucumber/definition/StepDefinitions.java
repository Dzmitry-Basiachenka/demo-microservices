...
public class StepDefinitions {
    ...
    private ResourceCompletedResponse resourceCompletedResponse;
    ...
    public void theFollowingResourcesAreSaved(List<Resource> resources) {
        resources.forEach(resource -> {
                ...
                assertThat(actualResource.getStorageId().equals(resource.storageId())).isTrue();
    ...
    @When("user completes resource download with id={long}")
    public void userCompletesResourceDownloadWithId(long id) {
        response = resourceClient.completeResourceUpload(id);

        resourceCompletedResponse = response.as(new TypeRef<>() {
        });
        assertThat(resourceCompletedResponse.id()).isEqualTo(id);
    }

    @And("resource completed response is")
    public void resourceCompletedResponseIs(String jsonResponse) throws JsonProcessingException {
        var expectedResponse = objectMapper.readValue(jsonResponse, new TypeReference<ResourceCompletedResponse>() {
        });
        assertThat(resourceCompletedResponse.id()).isEqualTo(expectedResponse.id());
        assertThat(resourceCompletedResponse.storageId()).isEqualTo(expectedResponse.storageId());
    }
